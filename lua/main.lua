...
if syntaxcheck then return end
  ...
  CHRONOS_COLOR_INTERVAL = 0.1 * (TIMER_INTERVAL/10) * CHRONOS_ACTIVATE_MULT
  CHRONOS_LIGHT_INTERVAL = 0.04 * (TIMER_INTERVAL/10) * CHRONOS_ACTIVATE_MULT
...
function state_ready()
    ...
    if((speed_slash_trigger or ((controller_table==nil) and isKeyPressed(1))) and (state_slash==0) and (enable_instant_slash>0)) then --check speed slash
     ...
     slash_counter = 0
      ...
      if(enable_slash_sfx>0) then
       if(SpEffect_Type_ptr>0) then
           ...
           was_SpEffect_successful = SpEffect_successful and writeInteger(SpEffect_ID_ptr,SP_EFFECT_ID) --divine confetti
     ...
     if(not SpEffect_successful) then print("could not apply SpEffect") end
